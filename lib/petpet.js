...
export async function GeneratePetpet(templateId, fromUserId, toUserId) {
    ...
    stream.on('data', (chunk) => {
        gifBuffer = Buffer.concat([gifBuffer, chunk])
    })
    ...
    encoder.setDelay(template.delay || 60) // frame delay in ms
    encoder.setQuality(1) // best quality (1-20)
    encoder.setDispose(2) // 2: Restore to background color
    encoder.setTransparent(0x000000) // 设置纯黑色为透明色
