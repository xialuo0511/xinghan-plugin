...
export async function GeneratePetpet(templateId, fromUserId, toUserId) {
    ...
    // 创建 GIF 编码器 - 使用 neuquant 算法支持 setTransparent
    ...
    for (let i = 0; i < frames.length; i++) {
        ...
        // 第二步：获取像素数据并将透明像素替换为标记色
        const imageData = transparentCtx.getImageData(0, 0, width, height)
        const data = imageData.data

        // 遍历所有像素，将透明像素(alpha < 128)替换为标记色
        for (let j = 0; j < data.length; j += 4) {
            if (data[j + 3] < 128) { // alpha 通道
                data[j] = TRANSPARENT_R     // R
                data[j + 1] = TRANSPARENT_G // G
                data[j + 2] = TRANSPARENT_B // B
                data[j + 3] = 255           // 设为不透明
            }
        }
        ...
        // 添加帧到 GIF
        encoder.addFrame(transparentCtx)
    }
