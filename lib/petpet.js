...
export async function GeneratePetpet(templateId, fromUserId, toUserId) {
    ...
    encoder.setDelay(template.delay || 60)  // 帧延迟 (ms)
    encoder.setQuality(10)  // 质量 (1-20, 越低越好)
    encoder.setTransparent(0x00FF00)  // 设置透明色键（绿色），alpha=0 的像素会自动使用此透明色
