...
export async function GeneratePetpet(templateId, fromUserId, toUserId) {
    ...
    // 使用 UPNG 编码为 APNG buffer
    const apngBuffer = UPNG.encode(frameBuffers, width, height, 0, delays)

    // 创建临时文件路径
    const tempDir = os.tmpdir()
    const tempFileName = `petpet-${Date.now()}-${Math.random().toString(36).slice(2)}.png`
    const tempFilePath = path.join(tempDir, tempFileName)

    let gifBuffer
    try {
        // 1. 写入临时文件 (fs/promises)
        await fsPromises.writeFile(tempFilePath, Buffer.from(apngBuffer))

        // 2. 使用 sharp 读取文件并转换为 GIF
        // animated: true, pages: -1 (确保读取所有帧)
        // limitInputPixels: false (防止大图报错)
        gifBuffer = await sharp(tempFilePath, {
            animated: true,
            pages: -1,
            limitInputPixels: false
        })
            .gif({ loop: 0, force: true }) // loop: 0 无限循环
            .toBuffer()

    } catch (err) {
        logger.error(`[星瀚插件] GIF 转换失败:`, err)
        throw err // 向上抛出以便调试
    } finally {
        // 3. 清理临时文件
        try {
            ...
            // 忽略清理错误
            logger.warn(`[星瀚插件] 临时文件清理失败: ${cleanupErr.message}`)
        }
    }
