...
export async function GeneratePetpet(templateId, fromUserId, toUserId) {
    ...
    // 透明标记色 (不常见的青色 R=1, G=255, B=1)
    const TRANSPARENT_R = 1
    const TRANSPARENT_G = 255
    const TRANSPARENT_B = 1
    ...
    encoder.setRepeat(0)
    // 设置透明标记色
    encoder.setTransparent((TRANSPARENT_R << 16) | (TRANSPARENT_G << 8) | TRANSPARENT_B)
    encoder.start()
